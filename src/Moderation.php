...
trait Moderation
{
    ...
    /**
     * The Moderation attributes that belong to the metadata.
     *
     * @var array
     */
    protected $metadata = [];
    ...
    /**
     * The Moderatable attributes that were modified.
     *
     * @var array
     */
    protected $modified = [];
    ...
    {
        return Config::get('moderation.drivers.database.connection');
    }
    ...
    {
        return Config::get('moderation.drivers.database.table', parent::getTable());
    }
    ...
    public function     resolveData(): array
    {
        ...
        // Metadata
        $this->data = [
            'moderation_id'         => $this->id,
            'moderation_event'      => $this->event,
            'moderation_url'        => $this->url,
            'moderation_ip_address' => $this->ip_address,
            'moderation_user_agent' => $this->user_agent,
            'moderation_tags'       => $this->tags,
            'moderation_created_at' => $this->serializeDate($this->created_at),
            'moderation_updated_at' => $this->serializeDate($this->updated_at),
        ...
        $flds = $this->fields;
        // Modified Moderatable attributes
        foreach ($this->fields as $field) {
            $this->data['new_'.$field->name] = $field->new;
        }

        foreach ($this->fields as $field) {
            $this->data['old_'.$field->name] = $field->old;
        }
    ...
    public function getDataValue(string $key)
    {
        ...
        // Moderatable value
        if ($this->moderatable && Str::startsWith($key, ['new_', 'old_'])) {
            $attribute = substr($key, 4);

            return $this->getFormattedValue(
                $this->moderatable,
                $attribute,
                $this->decodeAttributeValue($this->moderatable, $attribute, $value)
            );
        }
     ...
     *
     * @param Contracts\Moderatable $moderatable
     * @param string              $attribute
     * @param mixed               $value
     *
     * @return mixed
     */
    protected function decodeAttributeValue(Contracts\Moderatable $moderatable, string $attribute, $value)
    {
        $attributeModifiers = $moderatable->getAttributeModifiers();
    ...
    /**
     * Get the Moderation tags as an array.
     *
     * @return array
     */
